...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('query').setDescription('YouTube link or search string').setRequired(true),
    ),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      await interaction.editReply(`${result.track.title} has been added to the queue!`);
      setTimeout(async () => {
